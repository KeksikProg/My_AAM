...
def warp_piecewise_affine(image, src_points, dst_points, triangles, output_size):
    ...
    for tri_indices in triangles:
        ...
        # Проверка на вырожденные прямоугольники
        if rw == 0 or rh == 0:
            continue
        ...
        shifted_dst_tri = dst_tri - [x, y]
        cv2.fillConvexPoly(mask, np.int32(shifted_dst_tri), 1.0, 16, 0)
        ...
        if warped_crop.shape != mask.shape:
            continue  # защита от несовпадения размеров
